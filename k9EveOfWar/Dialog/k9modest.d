...
SAY     ~Bah, I've had enough with the fighting. Too many good folk lost to it cause they didn't know what they were doing. I'll have no part in that.~
       ++ ~You could try teaching them. I'm sure they could benefit from your experience.~ GOTO 11
...
IF ~~ THEN   BEGIN 11
	SAY ~And end up burying more of em?~
        ++ ~Look, you yourself admitted most of these folks are farmers by trade. The way I see it is you can help them to avoid that, or you could stand back and let it happen.~ GOTO 18
...
IF ~~ THEN  BEGIN 12
	SAY ~Bah, Kelddath may well be the finest man round these parts. Bit of an idealist, from what I hear, but he's as good in the heart as he is in the head.~
        ++ ~Look, most of these folks are farmers by trade. The way I see it you can help them to avoid their likely fate or you could stand back and let it happen.~ GOTO 18
...
SAY     ~Bah, like I said I've had enough with the fighting. Too many good folk lost to it cause they didn't know what they were doing. I'll have no more part in in.~
       ++ ~You could try teaching them.  I'm sure they could benefit from your experience.~ GOTO 11
       ++ ~Then teach them. They aren't going to get any better with just some stoggy leader like Kelddath or Darren leading them.~ GOTO 12
       ++ ~It's probably for the best. I doubt the watch needs such a weak spirited man leading it.~ GOTO 38
       ++ ~I haven't the time for this.~ GOTO 14
END
...
Funny thing she was. Leaftenders are that way I gather. Still, she proved helpful enough company. Even chose to join us for a time.~
        ...
        ++ ~What of Corness and Brae?~ GOTO 21
 	++ ~I've got better things to do than to listen to this.~ EXIT
END
...
IF ~~ THEN  BEGIN 33
	SAY ~Him again? I'm not interested in talking to him. Unless ye got a pint for me we don't have nothin to talk about.~ IF ~~ EXIT
END
...
IF ~~ THEN
BEGIN 37
	SAY ~Hehe. Yer down right wasted friend. Maybe go out...get some air. I'm sure you'll remember.~ IF ~~ THEN EXIT
         END


 IF ~~ THEN   BEGIN 38
	SAY ~Bunch o'bloody, soddering children around these parts I see. if ye had been wanting me to leave then ye should have just said so, saved us all the trouble.~ IF ~~ THEN DO ~AddJournalEntry(@1111,QUEST_DONE) SetGlobal("k9ModestTalk","GLOBAL",7) SetGlobal("ModestHelped","GLOBAL",7) EscapeArea()~  EXIT
END


IF WEIGHT #-99 ~NumTimesTalkedTo(0) Global("k9darrentalk", "GLOBAL", 0)~ THEN  BEGIN 39
	SAY ~*Hic*...what...what it is you want?~
      +~~+ ~Can't be talking to someone much without knowing their name. What's yours friend?~ GOTO 40
		++ ~My apologies, I didn't mean to disturb you.~  EXIT
END


IF ~~ THEN        BEGIN 40
	SAY ~My name? It's mind yer own damn business.~ IF ~~ THEN EXIT
END